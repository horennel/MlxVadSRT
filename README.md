...
- `--audio`: 输入音频文件路径。与 `--video`、`--srt` **互斥**，只能指定其中一个。
- `--video`: 输入视频文件路径。与 `--audio`、`--srt` **互斥**，只能指定其中一个。
- `--srt`: 输入已有 SRT 字幕文件路径（仅翻译模式）。与 `--audio`、`--video` **互斥**，**必须配合 `--to` 使用**。
...
- `--output`: 输出 SRT 文件名 (默认: `output.srt`)。使用 `--srt` 模式且未指定时，自动命名为 `原文件名.目标语言.srt`。
...
```
执行后会生成两个文件：`anime.srt`（原始日语字幕）和 `anime.zh.srt`（翻译后的中文字幕）。
...
- **翻译超时**: 如果翻译 API 响应较慢，可修改脚本中的 `TRANSLATE_API_TIMEOUT` 常量（默认 300 秒）。
- **台词遗漏**: 如果发现部分语音未被识别到，可降低脚本中的 `VAD_THRESHOLD` 值（默认 0.2，最低 0.1）。
